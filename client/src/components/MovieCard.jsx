...
const MovieCard = ({movie}) => {
    ...
    const runtimeLabel = Number(movie.runtime) > 0 ? timeFormat(movie.runtime) : 'Runtime TBA';
  return (
    <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-66'>
      ...
      <p className='text-sm text-gray-400 mt-2 '>
        {new Date(movie.release_date).getFullYear()} . {movie.genres.slice(0,2).map((genre)=>(genre.name)).join(' | ')}.{runtimeLabel}
      </p>
