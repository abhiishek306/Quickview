...
const AddShows = () => {
    ...
    const handleSubmit=async()=>{
        try{
            ...
            console.error('Submission error show:',error);
        ...
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
            {nowplayingMovies.map((movie)=>(
                ...
                <p className='flex items-center gap-1 text-gray-400'>
                    <StarsIcon className='w-4 h-4 text-primary fill-primary'/>{movie.vote_average.toFixed(1)}
                ...
                {Object.entries(dateTimeSelection).map(([date,times])=>(
                    <li key={date}>
                        ...
                        <div className='flex  flex-wrap gap-2 mt-1 text-sm'>
                            {times.map((time)=>(
                                <div key={time} className='border border-primary px-2 py-1 flex items-center rounded'><span>{time}</span>
